const NEWS_WORD_RE = /(新闻|資訊|资讯|快讯|快訊|热点|熱點|动态|動態|news)/i;
const AI_WORD_RE = /(AI|AIA|AHA|人工智能|大模型|模型|OpenAI|Claude|Gemini|智能体|智能體|机器人|機器人|芯片|晶片|算力)/i;
const AI_NEWS_FEEDS = [
...
export function isNewsQuestion(text) {
  ...
  return NEWS_WORD_RE.test(value) && (AI_WORD_RE.test(value) || /(美国|美國|中国|中國|中美|科技|互联网|互聯網|财经|財經|商业|商業)/.test(value));
}
...
function requestedCount(text) {
  ...
  if (/十[条條]|10[条條]/.test(text)) return 10;
...
function buildNewsQueries(text, now) {
  ...
  const topic = AI_WORD_RE.test(text) ? "AI 人工智能" : cleanFreeQuery(text);
  const queries = [];
...
function cleanFreeQuery(text) {
  return text
    .replace(/(帮我|请|给我|查|查询|总结|梳理|返回|重点|看看|看|新闻|资讯|快讯|热点|动态|条|前后|尽量)/g, " ")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 80) || "科技";
}
